...
public class Bartok : MonoBehaviour {
    ...
    {
        //if no number was given, then pick the next person
        if(num == -1)
        {
    ...
    {
        //see if we need to reshuffle the discard pile into the draw pile (AT ABSOLUTELY NO POINT WILL THIS RUN)
        //WHY DO I NEED TO SEE IF THERE SHOULD BE A SHUFFLE IN A GAME OVER CHECK???
        if(drawPile.Count == 0)
        {
    ...
    {
        if (CURRENT_PLAYER.type != PlayerType.human) return; //we have sentience
